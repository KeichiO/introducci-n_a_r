...
library(tidyverse) #UNA VEZ DESCARGADO EL Paquete solo se usa library(esta funcion) para llamar al paquete
...
##### ANOVA ##### 
# Subir datos
DPS <- read.csv("DesempenoProductivo_Soja.csv", dec=",")#dec igual entre comillapara que poder cambiar el punto por la coma en la planilla si es que da error
getwd() #para saber en qué carpeta esta ej introducci-n_a_r

#Los nombres de columnas tienen que seguir las buenas practicas 

#Explorar tabla
colnames(DPS) #nombre de columnas
head(DPS) #primeros datos de la tabla
unique(DPS$Variedad) #datos unicos
...
#Crear GGplot
# Se agrega el + para indicar que agregue el ggplot a un grafico boxplot, a diferencia del boxplot se una + en vez de coma
...
ggplot(DPS, aes(x = NGranos_m2,
                y = Rendimiento_Kgha,
                color = Variedad)) +
  geom_point() +  #OBJETO GEOMETRICO PARA CAMBIAR DE APARIENCIA EL GRAFICO
  xlim(0, 4000) +
  ggtitle("Rendimiento Kg/ha vs. Numero de Granos por m2") #TITULO
...
# lm fit es para modelos lineales
